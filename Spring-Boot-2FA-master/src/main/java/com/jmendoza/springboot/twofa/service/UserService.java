...
public class UserService {
    ...
    public boolean isLoginUser(User user){
    	 Optional<User> existUser = this.findByUsername(user.getUsername());
    	// return existUser.get();
    	 if(existUser.isPresent()) {
    		//return encoder().matches(user.getPassword(), encoder().encode(existUser.get().getPassword()));
    		return encoder().matches(user.getPassword(), existUser.get().getPassword());
    	 }
    	 return false;
    }
